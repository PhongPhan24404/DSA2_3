...
private:
    ...
    private:
        template <typename U>
        static auto test_equal(int) -> decltype(std::declval<U>() == std::declval<U>(), std::true_type());

        template <typename>
        static std::false_type test_equal(...);

        template <typename U>
        static auto test_ostream(int) -> decltype(std::declval<std::ostream&>() << std::declval<U>(), std::true_type());

        template <typename>
        static std::false_type test_ostream(...);

    public:
        static constexpr bool has_equal_operator = decltype(test_equal<T>(0))::value;
        static constexpr bool has_ostream_operator = decltype(test_ostream<T>(0))::value;
    };
...
protected:
    ...
    static bool equals(T &lhs, T &rhs, bool (*itemEqual)(T &, T &)) {
        ...
        else if constexpr (TypeTraits::has_equal_operator)
            return lhs == rhs;
        return false;
    }
...
XArrayList<T>::XArrayList(
    ...
    bool (*itemEqual)(T &, T &),
    int capacity) : capacity(capacity), count(0), deleteUserData(deleteUserData), itemEqual(itemEqual) {
    // TODO
    count = 0;
    data = new T[capacity];
}
...
XArrayList<T> &XArrayList<T>::operator=(const XArrayList<T> &list) {
    ...
    if (count > 0 && data != nullptr && data != list.data) {
        if (deleteUserData != 0) {
            deleteUserData(this);
        }
        delete[] data;
...
string XArrayList<T>::toString(string (*item2str)(T &)) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (item2str != 0) {
            ss << item2str(data[i]);
        } else if constexpr (TypeTraits::has_ostream_operator) {
            ss << data[i];
        }
    }
    ss << "]";
