...
template<class K, class V>
xMap<K,V>::xMap(const xMap<K,V>& map) : table(new DLinkedList<Entry*>[map.capacity]){
    copyMapFrom(map);
    deleteKeys=nullptr;
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V)) {
    ...
    for(auto pEntry: list){
        if(keyEQ(pEntry->key, key) && valueEQ(pEntry->value, value)){
            if(deleteKeyInMap != nullptr) deleteKeyInMap(pEntry->key);
            if(deleteValueInMap != nullptr) deleteValueInMap(pEntry->value);
            list.removeItem(pEntry, xMap<K,V>::deleteEntry);
            count--;
            return true;
        }
